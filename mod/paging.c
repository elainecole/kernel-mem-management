...
static void paging_vma_close(struct vm_area_struct * vma) {
  ...
  if (demand_paging == 0) { // pre-paging
    ...
    numpages = order / PAGE_SIZE;
    printk(KERN_INFO "in free pages order is %lu, numpages is %d, PAGE_SIZE is %lu\n", order, numpages, PAGE_SIZE);    
    pre_ptr = (struct page *) vma->vm_private_data; // retreive ptr to data struct state
    __free_pages(pre_ptr, my_get_order(numpages));
  } else { // demand paging
    ptr = (state *) vma->vm_private_data; // retreive ptr to data struct state
...
static int paging_mmap(struct file * filp, struct vm_area_struct * vma) {
  ...
  if (demand_paging == 0) { // pre-paging enabled
    ...
    order = (vma->vm_end - vma->vm_start);
    numpages = order / PAGE_SIZE;
    printk(KERN_INFO "order is %lu, numpages is %d, PAGE_SIZE is %lu, my get order is %d\n", order, numpages, PAGE_SIZE, my_get_order(numpages)); 
    page = alloc_pages(GFP_KERNEL, my_get_order(numpages));
    ...
    i = remap_pfn_range(vma, PAGE_ALIGN(vma->vm_start), page_to_pfn(page), PAGE_SIZE*numpages, vma->vm_page_prot);
    if (i == 0) { // success page table update
    } else {
      return -EFAULT;
    }
