...
static struct vm_operations_struct pre_paging_vma_ops = {
  .fault = pre_paging_vma_fault,
  .open  = pre_paging_vma_open,
  .close = pre_paging_vma_close
};
...
static int paging_mmap(struct file * filp, struct vm_area_struct * vma) {
  ...
  if(demand_paging == 1){
	...
	printk("Performing pre paging.");
 	vma->vm_ops = &pre_paging_vma_ops; 
  }
